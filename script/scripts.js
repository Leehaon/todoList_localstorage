...
function renderTodos(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    deleteBtn.onclick = () => renderTodos(index);   // 버튼을 클릭하면 removeIndex() 함수를 호출하겠다
    // argument로 index를 받았습니다. -> 즉 특정요소 하나만 삭제하는 버튼이라고 볼 수 있겠습니다.
    // deleteBtn.addEventListener('click', removeTodo(index));와 동의어
    // deleteBtn.addEventListener('click', function() {
    //   삭제하는 구현부; 와 같은 방식으로 이전에 작성해왔었습니다.
    // });
...
function removeTodo(index) {
  ...
  todo.splice(index, 1);   // 버튼이 눌러진 index넘버의 element만 삭제 -> 배열에서만 삭제
